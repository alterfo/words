...
(function() {
  var applicationCSSFiles, applicationJavaScriptFiles, browserSync, config, exec, g, gulp, ngAnnotate, onError, reload, runCommand, watchFiles;
  ...
  gulp.task('client-js:prod', function() {
    return gulp.src(applicationJavaScriptFiles).pipe(g.concat('application.min.js')).pipe(ngAnnotate()).pipe(g.uglify()).on('error', onError).pipe(gulp.dest('public/dist/'));
  });
